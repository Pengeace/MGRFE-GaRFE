# -*- coding: utf-8 -*-
"""
This script is for dealing with the dataset Prostate.
...
class APop(Population):
    def __init__(self, gene_len, potential_gene_rg, pop_size=90,
                 generation_size=3, pc=0.6, pm=0.2, best_size=5):
...
file_path = "D:\\codes\\python\\MGRFE\\Dataset One\\data\\Prostate.txt"
output_path = "D:\\codes\\python\\MGRFE\\Dataset One\\results\\"+dataset_name+"\\"+dataset_name+cur_time+"\\"
pos_name = "Normal"                        # the name of positive samples in the data set file
neg_name = "Tumour"                        # the name of negative samples in the data set file
ft_index_name = 'Genes'                    # the index name of the probe ID column
metrics_names = ["Sn", "Sp", "Acc", "Avc", "MCC"]   # the 5 metrics calculated of each GA individual
is_imbalanced = 0                       # whether the dataset is imbalanced

Total_layer = 3             # layer number
K = 5                       # K-fold CV
...
initial_ft_num = 700        # the length of the final feature range for the initialization of the first population

rfe_numbers = [3,2,4,1]     # GaRFE numbers in every layer
feature_numbers_layer =  [700,100,40,20]     # the size of candidate gene set in every layer
...
for l in range(Total_layer):
    ...
    for rfe_num in range(rfe_numbers[l]):
        rfe = RFE(dataset=dataset_name, initial_ft_rg=ft_rg_layer[l],
                  ...
                  pop_best_size=10,pop_size=100,best_size=35)
        rfe.down()
