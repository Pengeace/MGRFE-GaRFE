# -*- coding: utf-8 -*-
"""
This script is for dealing with the dataset ALL_AML.
...
with two functions of calc_fitness and calc_metrics helping calculate the fitness and classification metrics of each GA individual.
...
def calc_fitness(pop):          # the pop parameter is a list of gene combinations
    ...
    for gene_rg in pop:
        ...
        for train_index, test_index in skf.split(X_cur, y):
            ...
            result = confusion_matrix(y_train, predicts, labels=[0, 1])
            mt_clf_te += calc_metrics(result)
        indiv.metrics_ta = list(mt_clf_ta / k)
        indiv.metrics_te = list(mt_clf_te / k)
        indiv.clf = clf
        indiv.acc = 0.4 * indiv.metrics_ta[2] + 0.6 * indiv.metrics_te[2]
...
class RFE():
    ...
    def __init__(self, dataset, initial_ft_rg, cur_layer, rfe_num, file_path, output_path, max_gene_len,
                 pop_best_size=5, pop_size=95, best_size=30):
...
file_path = "D:\\codes\\python\\MGRFE\\Dataset Two\\data\\ALL_AML.txt"
...
layer_best_size = 80        # the size of stored best GA individuals in each layer of MGRFE
...
rfe_numbers = [2,2,1,3]     # GaRFE numbers in every layer
feature_numbers_layer = [500,100,60,20]      # the size of candidate gene set in every layer
rfe_feature_number_layer = [70,30,20,15]    # the initial gene number of each individual in each layer
...
for l in range(Total_layer):
    ...
    for rfe_num in range(rfe_numbers[l]):
        rfe = RFE(dataset=dataset_name, initial_ft_rg=ft_rg_layer[l],
                  ...
                  pop_best_size=10,pop_size=85,best_size=25)
        rfe.down()
